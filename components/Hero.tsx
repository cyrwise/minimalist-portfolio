...
export default function Hero() {
  const scrollToNext = () => {
    ...
    if (nextSection) {
      const y = nextSection.getBoundingClientRect().top + window.scrollY - 120;
  ...
  return (
    <motion.section 
      ...
      animate="visible"
      variants={fadeUp}
      className="relative min-h-[calc(100vh-140px)] pt-16 flex flex-col justify-start border-none px-4 md:px-0"
    >
      <div className="flex flex-col-reverse md:flex-row gap-12 items-center justify-between w-full">
        <div className="md:w-3/5 text-center md:text-left z-10 relative">
          ...
          <h2 className="text-5xl md:text-7xl lg:text-[5.5rem] font-medium leading-none mb-8 text-fg font-serif tracking-tight">
            Crafting systems <br /> & scaling ventures.
          </h2>
          
          <p className="text-lg md:text-xl max-w-2xl text-muted font-normal leading-relaxed mb-10 mx-auto md:mx-0">
            I am a builder and founder exploring the intersection of technology, business, and philosophy. Currently at UC Berkeley, my focus is on engineering intelligent products and driving growth. I aim to connect robust technical architecture with visionary strategy.
          </p>
      ...
      <motion.div 
        ...
        className="mt-auto pt-12 pb-4 mx-auto flex flex-col items-center cursor-pointer hover:opacity-100 transition-all"
        onClick={scrollToNext}
      >
        <span className="text-[10px] uppercase tracking-[0.2em] font-mono mb-2 text-muted">Explore</span>
        <motion.svg 
          animate={{ y: [0, 6, 0] }} 
